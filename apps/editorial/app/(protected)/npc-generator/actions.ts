...
export async function generateNpcProfile(formData: FormData) {
  ...
  const selectedModules = moduleIds.map((id) => modules?.find((module) => module.id === id)).filter(Boolean) as { id: string; title: string }[];
...
MODULE, IN DENEN DER AKTEUR AUFTRETEN DARF: ${selectedModules.map((module) => module.title).join(", ")}
KONTEXT: ${context || "Digitaler Schulalltag und Feed"}

WICHTIG:
- Der Akteur-Typ ist eine Weltrolle, keine Glaubwürdigkeitswertung.
- Die drei ausgewählten Interessen sind die kanonischen Interessen und sollen Persona, Themen und Verhalten prägen.
- Bei News-Outlets, Marken, Organisationen, Communities und Bots keine künstliche Teenager-Persona erzwingen; entwickle stattdessen passende redaktionelle, kommerzielle oder soziale Eigenschaften.
...
export async function generateNpcPosts(formData: FormData) {
  ...
  if (!runtime?.npc) throw new Error("Für diesen Akteur existiert in der Instanz noch kein Runtime-Profil.");
  ...
  const key = requireKey(); const npc = runtime.npc;
  ...
  const { error } = await supabase.from("content_items").insert(rows); if (error) throw new Error(error.message);
...
export async function generateNpcReaction(formData: FormData) {
  const supabase = supabaseServerClient();
  const npcId = String(formData.get("npcId") ?? ""); const classInstanceId = String(formData.get("classInstanceId") ?? ""); const contentItemId = String(formData.get("contentItemId") ?? "");
  ...
  if (!runtime?.npc || !post) throw new Error("Akteur oder Beitrag nicht in dieser Instanz gefunden.");
  ...
  const key = requireKey(); const npc = runtime.npc;
  const result = await gemini(key, `Entscheide, wie dieser Akteur auf einen Social-Media-Beitrag reagieren würde. TYP: ${npc.actor_type}\nAKTEUR: ${npc.display_name}\nINTERESSEN: ${(npc.interests ?? []).join(", ")}\nINTERESSEN-SCHLÜSSEL: ${(npc.interest_keys ?? []).join(", ")}\nPERSONA: ${JSON.stringify(npc.persona)}\nSTIMME: ${JSON.stringify(npc.voice)}\nZUSTAND: ${JSON.stringify(runtime.current_state)}\nERINNERUNGEN: ${JSON.stringify(memories ?? [])}\nBEITRAG: ${post.body}\n\nNicht jeder Beitrag bekommt eine Reaktion.`, REACTION_SCHEMA);
