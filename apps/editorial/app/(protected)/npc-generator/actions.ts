...
async function gemini(apiKey: string, prompt: string, schema: object): Promise<GeminiResult> {
  try {
    return { result: await geminiRequest(apiKey, "gemini-3.7-flash", prompt, schema), model: "gemini-3.7-flash" };
  } catch (primaryError) {
    const message = primaryError instanceof Error ? primaryError.message : String(primaryError);
    const highDemand = /high demand|503|unavailable|temporarily/i.test(message);
    if (!highDemand) throw new Error(`Gemini API Fehler: ${message}`);
    try {
      return { result: await geminiRequest(apiKey, "gemini-3.6-flash", prompt, schema), model: "gemini-3.6-flash" };
    } catch (fallbackError) {
      const fallbackMessage = fallbackError instanceof Error ? fallbackError.message : String(fallbackError);
      throw new Error(`Gemini API Fehler: 3.7 Flash nicht verfügbar; Fallback 3.6 Flash ebenfalls fehlgeschlagen: ${fallbackMessage}`);
    }
  }
}
...
export async function generateNpcProfile(formData: FormData) {
  ...
  await supabase.from("npc_generation_runs").insert({ class_instance_id: null, npc_id: npc.id, generation_type: "profile", keywords, context, provider: "gemini", model: generated.model, status: "draft", output: { ...result, actorType, selectedInterestKeys: interestKeys, selectedModuleIds: moduleIds } });
...
export async function generateNpcPosts(formData: FormData) {
  ...
  await supabase.from("npc_generation_runs").insert({class_instance_id:classInstanceId,npc_id:npcId,generation_type:"posts",keywords:npc.keywords??[],context:npc.context??"",provider:"gemini",model:generated.model,status:"draft",output:result}); revalidatePath("/npc-generator");
}
...
export async function generateNpcReaction(formData: FormData) {
  ...
  const {data:memories}=await supabase.from("npc_memory").select("memory_type, content, salience").eq("npc_instance_id",runtime.id).order("created_at",{ascending:false}).limit(10); const key=requireKey(); const npc=runtime.npc; const generated=await gemini(key,`Entscheide, wie dieser Akteur auf einen Social-Media-Beitrag reagieren würde. TYP: ${npc.actor_type}\nAKTEUR: ${npc.display_name}\nINTERESSEN: ${(npc.interests??[]).join(", ")}\nINTERESSEN-SCHLÜSSEL: ${(npc.interest_keys??[]).join(", ")}\nPERSONA: ${JSON.stringify(npc.persona)}\nSTIMME: ${JSON.stringify(npc.voice)}\nZUSTAND: ${JSON.stringify(runtime.current_state)}\nERINNERUNGEN: ${JSON.stringify(memories??[])}\nBEITRAG: ${post.body}\n\nNicht jeder Beitrag bekommt eine Reaktion.`,REACTION_SCHEMA); const result=generated.result; if(result.action==="ignore") return;
