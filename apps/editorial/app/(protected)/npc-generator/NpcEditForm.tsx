...
export function NpcEditForm({ npc, interests, modules, assignedModuleIds }: { npc: Npc; interests: Interest[]; modules: Module[]; assignedModuleIds: string[] }) {
  ...
  function submit(event: React.FormEvent<HTMLFormElement>) {
    ...
    startTransition(async () => {
      try {
        await updateNpcProfile(data);
        setSaved(true);
      } catch (err) {
        setError(err instanceof Error ? err.message : "Der Akteur konnte nicht gespeichert werden.");
      }
    });
  }
  ...
  return (
    <details className="group rounded-xl border border-slate-200 bg-slate-50">
      <summary className="cursor-pointer list-none px-4 py-3 flex items-center justify-between gap-3 text-sm font-semibold text-slate-700">
        <span>✎ Akteur bearbeiten</span>
        <span className="text-[11px] text-slate-400 group-open:hidden">Details öffnen</span>
      </summary>
      <form onSubmit={submit} className="border-t border-slate-200 p-4 space-y-4 bg-white rounded-b-xl">
        ...
        <div className="rounded-lg bg-violet-50 px-3 py-2 text-xs text-violet-800">Hier änderst du die redaktionellen Eigenschaften des Akteurs. Module und Interessen bestimmen, wo und bei welchen Themen er später berücksichtigt werden kann.</div>
        ...
        <label className="block"><span className="label">MODULE</span><select name="moduleIds" multiple defaultValue={assignedModuleIds} className="control min-h-32">{modules.map(m => <option key={m.id} value={m.id}>{m.title}</option>)}</select><span className="text-[11px] text-slate-400">Mehrere Module mit ⌘/Strg auswählen. Mindestens eines ist erforderlich.</span></label>
