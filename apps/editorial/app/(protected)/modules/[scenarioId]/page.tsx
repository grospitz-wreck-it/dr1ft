...
const TYPE_OPTIONS = [
  ["post", "Post"],
  ["comment", "Kommentar"],
  ["dm_message", "Direktnachricht"],
  ["minigame", "Minigame"],
  ["reflection_prompt", "Reflexion"],
] as const;

const FACTORS = [
  ["reward", "Gewinnversprechen"],
  ["time_pressure", "Zeitdruck"],
  ["emotion", "Emotion"],
  ["source_clarity", "Quellenklarheit"],
...
function dimensionEntries(profile: any) {
  return Object.entries((profile?.dimensions ?? {}) as Record<string, unknown>)
    .filter(([, value]) => typeof value === "number") as [string, number][];
}

function effectiveValue(profile: any, overrides: Record<string, unknown>, key: string) {
  return score(overrides?.[key]) ?? score(profile?.dimensions?.[key]);
}
...
export default async function ModuleBuilderPage({ params }: { params: { scenarioId: string } }) {
  ...
  if (!module) {
    return <div className="p-8 text-sm text-slate-500">Modul nicht gefunden.</div>;
  }

  return (
    <div className="px-6 py-5 max-w-6xl">
      <div className="flex items-center gap-2 text-xs text-slate-400 mb-4">
        <Link href="/modules" className="hover:text-slate-700 flex items-center gap-1"><ArrowLeft className="w-3.5 h-3.5" /> Module</Link>
      ...
      <div className="flex items-start justify-between gap-6 mb-6">
        <div>
          <div className="flex items-center gap-3">
            <h1 className="text-xl font-semibold text-slate-900">{module.title}</h1>
            <span className={`text-xs ${module.is_active ? "text-status-live" : "text-slate-400"}`}>{module.is_active ? "● aktiv" : "○ Entwurf"}</span>
          </div>
          <p className="text-sm text-slate-500 mt-1 max-w-2xl">{module.description || "Noch keine Beschreibung."}</p>
          <div className="flex gap-4 mt-3 text-xs text-slate-400">
            <span>{blocks?.length ?? 0} Bausteine</span><span>{missionCount ?? 0} Lernaufgaben</span><span>{module.age_rating}</span>
          </div>
        </div>
        <form action={toggleModule.bind(null, scenarioId, !module.is_active)}>
          <button className="border border-border rounded-md px-3 py-2 text-sm hover:bg-panel">{module.is_active ? "Modul deaktivieren" : "Modul aktivieren"}</button>
        </form>
      </div>

      <section className="bg-panel border border-border rounded-xl p-5 mb-6">
        <div className="flex items-center gap-2 mb-4"><Sparkles className="w-4 h-4 text-accent" /><h2 className="font-semibold text-sm text-slate-900">Modul-Einstellungen</h2></div>
        <form action={updateModule.bind(null, scenarioId)} className="grid grid-cols-1 md:grid-cols-3 gap-3">
          ...
          <div className="flex gap-2">
            <select name="ageRating" defaultValue={module.age_rating} className="border border-border rounded-md px-3 py-2 text-sm flex-1">
              <option value="all_ages">Alle</option><option value="12_plus">12+</option><option value="16_plus">16+</option>
            </select>
            <button className="bg-accent text-white rounded-md px-4 text-sm">Speichern</button>
          </div>
        </form>
      </section>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-[minmax(0,1fr)_360px] gap-6 items-start">
        <section>
          <div className="flex items-center justify-between mb-3">
            <div><h2 className="font-semibold text-slate-900">Bausteine</h2><p className="text-xs text-slate-400">Der Default kommt aus dem Interaktionsprofil. Nur konkrete Abweichungen werden überschrieben.</p></div>
          </div>
          ...
          <div className="space-y-4">
            {(blocks ?? []).map((block: any, index: number) => {
              ...
              return (
                <form key={block.id} action={updateModuleBlock.bind(null, block.id, scenarioId)} className="bg-panel border border-border rounded-xl p-5">
                  <div className="flex items-start justify-between gap-4 mb-4">
                    <div className="flex items-start gap-3">
                      <div className="w-7 h-7 rounded-full bg-canvas border border-border flex items-center justify-center text-xs font-medium text-slate-500">{index + 1}</div>
                      <div>
                        <div className="flex items-center gap-2"><h3 className="font-medium text-slate-900">{block.title}</h3><span className="text-[11px] text-slate-400">{block.type}</span></div>
                        <p className="text-xs text-slate-400 mt-0.5">{block.status === "live" ? "● live" : "○ Entwurf"}{profile ? ` · ${profile.label}` : " · kein Profil"}</p>
                      </div>
                    </div>
                    <div className="flex gap-2">
                      <button formAction={toggleBlockLive.bind(null, block.id, scenarioId, block.status !== "live")} className="text-xs border border-border rounded-md px-2 py-1 hover:bg-canvas">{block.status === "live" ? "Deaktivieren" : "Live schalten"}</button>
                      <button formAction={deleteModuleBlock.bind(null, block.id, scenarioId)} className="text-xs text-status-rejected px-2 py-1">Löschen</button>
                    </div>
                  </div>

                  <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                    <input name="title" defaultValue={block.title ?? ""} required className="border border-border rounded-md px-3 py-2 text-sm" placeholder="Titel" />
                    <select name="interaction_profile_id" defaultValue={block.interaction_profile_id ?? ""} className="border border-border rounded-md px-3 py-2 text-sm">
                      <option value="">Kein Interaktionsprofil</option>
                      {(profiles ?? []).map((p: any) => <option key={p.id} value={p.id}>{p.label}</option>)}
                    </select>
                    <textarea name="body" defaultValue={block.body ?? ""} className="border border-border rounded-md px-3 py-2 text-sm min-h-24 md:col-span-2" placeholder="Inhalt / Text" />
                    <input name="techniques" defaultValue={techniques} className="border border-border rounded-md px-3 py-2 text-sm" placeholder="Manipulationstechniken, kommagetrennt" />
                    <select name="difficulty" defaultValue={String(block.difficulty ?? 1)} className="border border-border rounded-md px-3 py-2 text-sm"><option value="1">Schwierigkeit 1</option><option value="2">Schwierigkeit 2</option><option value="3">Schwierigkeit 3</option><option value="4">Schwierigkeit 4</option><option value="5">Schwierigkeit 5</option></select>
                  </div>

                  <div className="mt-4 border-t border-border pt-4">
                    <div className="flex items-center justify-between mb-2"><p className="text-xs font-medium text-slate-600">Situationsfaktoren</p><span className="text-[11px] text-slate-400">1 niedrig · 5 hoch</span></div>
                    <div className="grid grid-cols-2 md:grid-cols-5 gap-2">
                      {FACTORS.map(([key, label]) => <label key={key} className="text-[11px] text-slate-500"><span className="block mb-1">{label}</span><input name={key} type="number" min={1} max={5} placeholder="Default" defaultValue={typeof overrides[key] === "number" ? String(overrides[key]) : ""} className="border border-border rounded-md px-2 py-1.5 w-full text-sm" /></label>)}
                    </div>
                  </div>

                  <div className="mt-4 border-t border-border pt-4 flex flex-wrap items-end gap-4">
                    <div className="min-w-[220px]"><p className="text-xs font-medium text-slate-600 mb-2">Ziel-Kompetenzen</p>{(competencies ?? []).map((c: any) => <label key={c.id} className="mr-3 inline-flex items-center gap-1 text-xs text-slate-500 mb-1"><input type="checkbox" name="target_competencies" value={c.id} defaultChecked={(block.target_competencies ?? []).includes(c.id)} />{c.title}</label>)}</div>
                    <div className="flex gap-2 ml-auto"><select name="ageRating" defaultValue={block.age_rating ?? module.age_rating} className="border border-border rounded-md px-2 py-1.5 text-xs"><option value="all_ages">Alle</option><option value="12_plus">12+</option><option value="16_plus">16+</option></select><button className="bg-accent hover:bg-accent-hover text-white text-xs px-3 py-1.5 rounded-md">Baustein speichern</button></div>
                  </div>
        ...
        <aside className="space-y-4 xl:sticky xl:top-5">
          ...
          <div className="bg-canvas border border-border rounded-xl p-4 text-xs text-slate-500 space-y-2">
            <p className="font-medium text-slate-700">So funktioniert die Bewertung</p>
            <p>Das Interaktionsprofil liefert den Standard. Situationsfaktoren beschreiben den konkreten Kontext. Nur wenn eine Situation abweicht, überschreibst du den Default.</p>
            <p className="text-slate-400">Beispiel: „Teilen“ bleibt automatisch mit seinem Standardprofil bewertet. Ein besonders emotionaler, zeitkritischer Post kann zusätzlich Emotion 5 und Zeitdruck 5 erhalten.</p>
          </div>
