...
- Nutze Anglizismen, Abkürzungen, Neologismen, Eindeutschungen, Slang, Füllwörter, Fragmente, Übertreibungen, Ironie und Meme-/Gaming-Referenzen nur, wenn Kontext und Stimme sie tragen.
- Tippfehler sind seltene natürliche Vertipper, keine absichtlich schlechte Rechtschreibung.
- Lowercase, fehlende Satzzeichen oder verkürzte Wörter können vorkommen.
...
function extractText(data: any) {
  ...
  return parts.map((part: any) => part?.text ?? part?.content?.find?.((p: any) => typeof p?.text === "string")?.text ?? "").filter(Boolean).join("\n");
}
...
async function generateGeminiText(apiKey: string, prompt: string) {
  ...
  for (const model of models) {
    const response = await fetch("https://generativelanguage.googleapis.com/v1beta/interactions", {
      ...
      body: JSON.stringify({ model, input: prompt, response_format: { type: "text", mime_type: "application/json", schema: { type: "array", items: { type: "object" } } } }),
    });
    if (response.ok) {
      const data = await response.json();
      const raw = extractText(data);
      if (!raw) throw new Error("Gemini hat keine Textausgabe geliefert.");
      return { items: parseItems(raw), model };
    }
...
function pickImageIndexes(count: number, ratio: number, mode: string, items: any[]) {
  ...
  return new Set(candidates.slice(0, target).map((candidate) => candidate.index));
}
...
export async function generateAmbientDrafts(formData: FormData) {
  ...
  const model = text(formData, "model", "gemini");
  ...
  const provider = model === "claude" ? "claude" : "gemini";
  ...
  if (provider === "claude") {
    ...
    items = parseItems((await response.json()).content?.[0]?.text?.trim() ?? "[]");
  } else {
    const result = await generateGeminiText(apiKey, prompt);
    items = result.items;
    usedModel = result.model;
  }
  if (!Array.isArray(items) || !items.length) throw new Error(`Keine Ambient-Items generiert. ${provider === "gemini" ? "Gemini hat geantwortet, aber keine verwertbaren Items geliefert." : "Bitte erneut versuchen."}`);
  ...
  for (let index = 0; index < items.length; index++) {
    ...
    rows.push({ type: "post", scenario_id: null, creator_id: creatorId, body: item.body.trim().slice(0, 1000), media_url: mediaUrl, media_type: mediaUrl ? "image" : null, manipulation_techniques: [], target_competencies: [], difficulty: 1, age_rating: ageBand === "16_17" || ageBand === "18_plus" ? "16_plus" : ageBand === "all" ? "all_ages" : "12_plus", status: "draft", extra: { generatedBy: "ai", generatorVersion: "ambient-studio-v4", provider, model: usedModel, theme, ageBand, interests, style, styleLabel: styleText, typoLevel, slangLevel, emojiLevel, imageRatio, imageMode, format: item.format, mood: item.mood, topic: item.topic, creatorVibe: item.creatorVibe, imageRequested: shouldImage, imageGenerated: Boolean(mediaUrl), imageModel: mediaUrl ? "gemini-3.1-flash-image" : null, languageLibrary: "source-grounded-2026-08" } });
  }
