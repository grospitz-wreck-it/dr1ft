...
export async function createScenario(formData: FormData) {
  ...
  const { data: scenario, error } = await supabase.from("scenarios").insert({ title, description, age_rating: ageRating, slug, is_active: false }).select().single();
...
export async function toggleScenarioActive(scenarioId: string, nextActive: boolean) {
  const supabase = supabaseServerClient();
  const { error } = await supabase.from("scenarios").update({ is_active: nextActive }).eq("id", scenarioId);
...
export async function createContentItem(scenarioId: string, formData: FormData) {
  ...
  if (isAmbient && manipulationTechniques.length > 0) throw new Error("Ambient-Content darf keine Manipulationstechniken tragen");
  ...
  const { error } = await supabase.from("content_items").insert({
    ...
    status: "draft",
    extra: { ...(baseEngagement > 0 ? { baseEngagement } : {}), ...(baseCommentCount > 0 ? { baseCommentCount } : {}) },
  });
  if (error) throw new Error(error.message);
  revalidatePath(`/scenarios/${scenarioId}`);
}
...
const ALLOWED_TRANSITIONS: Record<string, string[]> = {
  draft: ["in_review", "archived"], in_review: ["approved", "rejected", "draft"], approved: ["live", "in_review"], live: ["archived"], rejected: ["draft"], archived: ["draft"],
};

export async function updateContentItemStatus(contentItemId: string, currentStatus: string, nextStatus: string, reviewNotes?: string) {
  const supabase = supabaseServerClient();
  const allowed = ALLOWED_TRANSITIONS[currentStatus] ?? [];
  if (!allowed.includes(nextStatus)) throw new Error(`Übergang ${currentStatus} -> ${nextStatus} ist nicht erlaubt`);
