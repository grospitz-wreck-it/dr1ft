...
import { Sparkles, Image as ImageIcon, Wand2, Zap } from "lucide-react";
...
export default async function AmbientContentPage() {
  ...
  const grouped = STATUS_ORDER.map((status) => ({ status, items: (items ?? []).filter((c) => c.status === status) }));
    ...
    <div className="min-h-screen bg-slate-50 px-6 py-6">
      <div className="max-w-7xl mx-auto space-y-6">
        <header className="flex items-end justify-between gap-6">
          <div>
            <div className="flex items-center gap-2 text-accent text-xs font-semibold uppercase tracking-widest mb-2"><Sparkles className="w-4 h-4" /> AI Content Studio</div>
            <h1 className="text-3xl font-semibold tracking-tight text-slate-900">Ambient-Content-Generator</h1>
            <p className="text-slate-500 mt-2 max-w-3xl">Erzeuge einen ganzen lebendigen Feed statt einfach nur eine Liste von Posts. Zielgruppe, Interessen, Stimmen, Schreibfehler, Slang, Emojis, Formate und Bilder werden als gemeinsames Generationsprofil behandelt.</p>
          </div>
          <div className="hidden md:flex gap-2 text-xs"><Stat label="LIVE" value={liveCount} /><Stat label="DRAFTS" value={draftCount} /><Stat label="BILDER" value={imageCount} /></div>
        </header>
        ...
        <form action={generateAmbientDrafts} className="grid xl:grid-cols-[1.45fr_1fr] gap-5">
          <section className="bg-white border border-slate-200 rounded-2xl p-6 shadow-sm space-y-6">
            <div className="flex items-center justify-between"><div><h2 className="font-semibold text-slate-900 flex items-center gap-2"><Wand2 className="w-4 h-4 text-accent" /> Feed-Rezept</h2><p className="text-xs text-slate-500 mt-1">Was soll die Engine heute für uns bauen?</p></div><span className="text-[11px] px-2 py-1 rounded-full bg-violet-50 text-violet-700 font-medium">AMBIENT v2</span></div>
            ...
            <div className="grid md:grid-cols-2 gap-4">
              <Field label="ZIELGRUPPE"><select name="ageBand" defaultValue="14_15" className="control"><option value="12_13">12–13 · Early Teen</option><option value="14_15">14–15 · Teen</option><option value="16_17">16–17 · Older Teen</option><option value="18_plus">18+ · Young Adult</option><option value="all">Altersneutral</option></select></Field>
              <Field label="CREATOR / STIMME"><select name="creatorId" className="control"><option value="">Automatisch wechseln</option>{ambientCreators?.map((creator) => <option key={creator.id} value={creator.id}>{creator.display_name}</option>)}</select></Field>
            </div>
            ...
            <div className="grid md:grid-cols-3 gap-4"><SliderSelect name="typoLevel" label="TIPPFEHLER" options={["0 · clean", "1 · gelegentlich", "2 · glaubwürdig", "3 · chaotisch"]} defaultValue="1" /><SliderSelect name="slangLevel" label="JUGENDSPRACHE" options={["0 · neutral", "1 · leicht", "2 · natürlich", "3 · stark"]} defaultValue="2" /><SliderSelect name="emojiLevel" label="EMOJIS" options={["0 · keine", "1 · sparsam", "2 · natürlich", "3 · viel"]} defaultValue="2" /></div>

            <div className="grid md:grid-cols-2 gap-4"><Field label="ANZAHL"><select name="count" defaultValue="20" className="control">{[5,10,20,30,50].map((n) => <option key={n} value={n}>{n} Items</option>)}</select></Field><Field label="TEXT-MODELL"><select name="model" defaultValue="claude" className="control"><option value="claude">Claude · schnell & textstark</option><option value="gemini">Gemini · multimodal</option></select></Field></div>
          </section>

          <section className="bg-slate-900 text-white rounded-2xl p-6 shadow-sm space-y-6">
            <div className="flex items-center justify-between"><div><h2 className="font-semibold flex items-center gap-2"><ImageIcon className="w-4 h-4" /> Visual Engine</h2><p className="text-xs text-slate-400 mt-1">Gemini erzeugt echte Feed-Bilder aus den generierten Ideen.</p></div><span className="text-[11px] px-2 py-1 rounded-full bg-white/10 text-slate-300">NANO BANANA</span></div>
            <FieldDark label="BILDSTRATEGIE"><select name="imageMode" defaultValue="smart" className="darkControl"><option value="none">Keine Bilder</option><option value="some">Gelegentlich · 1 von 3</option><option value="smart">Smart · KI entscheidet</option><option value="all">Jeder Post mit Bild</option></select></FieldDark>
            ...
            <div className="rounded-xl border border-white/10 bg-white/5 p-4 space-y-3"><div className="flex items-center gap-2 text-sm font-medium"><Zap className="w-4 h-4 text-yellow-300" /> Authenticity Engine</div><div className="text-xs text-slate-400 leading-5">Die Engine variiert Länge, Satzbau, Groß-/Kleinschreibung, Emoji-Dichte, Tippfehler, Slang, Formate und Stimmungen. Dadurch sieht der Feed nicht aus wie 20 Kopien desselben KI-Prompts.</div></div>
            ...
            <button type="submit" className="w-full rounded-xl bg-white text-slate-900 hover:bg-slate-100 px-5 py-3 font-semibold text-sm flex items-center justify-center gap-2"><Sparkles className="w-4 h-4" /> Ambient Feed generieren</button>
        ...
        <section className="grid xl:grid-cols-2 gap-5">{grouped.map((group) => group.items.length > 0 && <div key={group.status} className="space-y-2"><div className="flex items-center justify-between px-1"><h2 className="text-xs font-semibold tracking-widest text-slate-500">{statusLabel[group.status]} · {group.items.length}</h2></div><ul className="space-y-2">{group.items.map((item) => <li key={item.id} className="bg-white border border-slate-200 rounded-xl overflow-hidden">{item.media_url && <img src={item.media_url} alt="" className="w-full max-h-80 object-cover" />}<div className="p-4 space-y-3"><p className="text-sm leading-6 text-slate-900">{item.body}</p>{item.extra?.generatedBy === "ai" && <div className="flex flex-wrap gap-1.5 text-[10px] text-slate-500"><Tag>{item.extra?.ageBand}</Tag><Tag>{item.extra?.styleLabel}</Tag>{item.extra?.format && <Tag>{item.extra.format}</Tag>}{item.extra?.topic && <Tag>{item.extra.topic}</Tag>}{item.extra?.imageGenerated && <Tag>🖼 Gemini</Tag>}</div>}<div className="flex items-center justify-between gap-2"><ContentStatusControl contentItemId={item.id} status={item.status} />{item.status !== "archived" && <form action={archiveAmbientContent.bind(null, item.id)}><button type="submit" className="text-xs text-slate-400 hover:text-slate-700">Archivieren</button></form>}</div></div></li>)}</ul></div>)}</section>
      </div>
      <style>{`.control{width:100%;border:1px solid #e2e8f0;border-radius:12px;padding:10px 12px;font-size:13px;background:white;color:#0f172a}.darkControl{width:100%;border:1px solid rgba(255,255,255,.12);border-radius:12px;padding:10px 12px;font-size:13px;background:rgba(255,255,255,.06);color:white}`}</style>
    </div>
  );
}
...
function SliderSelect({ name, label, options, defaultValue }: { name: string; label: string; options: string[]; defaultValue: string }) { return <Field label={label}><select name={name} defaultValue={defaultValue} className="control">{options.map((option, index) => <option key={option} value={index}>{option}</option>)}</select></Field>; }
