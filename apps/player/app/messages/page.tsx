...
export default async function MessagesInboxPage() {
  ...
  const { data: npcCreators } = await supabase.from("creators").select("id, display_name, handle").eq("kind", "npc").in("scenario_id", scopedScenarioIds);
  const { data: groupChats } = await supabase.from("group_chats").select("id, title").eq("status", "live").in("scenario_id", scopedScenarioIds);
  const { data: pending } = await supabase.from("user_npc_conversations").select("creator_id, pending_resume_at").eq("user_id", user.id).eq("class_instance_id", classInstanceId);
  const pendingByCreator = new Map((pending ?? []).map((p) => [p.creator_id, p.pending_resume_at]));

  return <main className="min-h-screen bg-ink"><header className="sticky top-0 z-10 bg-ink/90 backdrop-blur border-b border-ink-border safe-top px-4 py-3 flex items-center justify-between"><a href="/feed" className="touch-target inline-flex items-center gap-1 font-mono text-xs text-ash"><ChevronLeft className="w-4 h-4" /> Feed</a><p className="font-display text-paper text-sm">Nachrichten</p><span className="w-11" /></header><div className="max-w-md mx-auto px-3 py-4 space-y-6">
    ...
    <section><p className="font-mono text-[11px] text-ash uppercase tracking-wide mb-2">Direktnachrichten</p><div className="space-y-2">{npcCreators?.map((c) => { const resumeAt = pendingByCreator.get(c.id); const isDue = resumeAt && new Date(resumeAt) <= new Date(); return <a key={c.id} href={`/messages/${c.id}`} className="flex items-center justify-between bg-paper text-ink rounded-card px-4 py-3 text-sm touch-target"><span className="flex items-center gap-2"><Send className="w-4 h-4 text-ink/40 shrink-0" />{c.display_name} <span className="text-ink/40">{c.handle}</span></span>{isDue && <span className="flex items-center gap-1 text-xs text-red-500"><Circle className="w-2 h-2 fill-current" /> neu</span>}</a>; })}{(!npcCreators || npcCreators.length === 0) && <p className="text-ash text-sm">Noch keine Kontakte.</p>}</div></section>
  </div></main>;
}
