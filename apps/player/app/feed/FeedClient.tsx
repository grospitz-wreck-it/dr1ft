...
export function FeedClient({ initialItems, userId, classInstanceId, likedContentIds, competencyDisplay, profile }: {
  ...
  return (
    <main className="min-h-screen bg-ink">
      <header className="sticky top-0 z-10 bg-ink/90 backdrop-blur border-b border-ink-border safe-top px-4 py-2 flex items-center justify-between">
        <ProfileBadge displayName={profile.displayName} username={profile.username} avatarSeed={profile.avatarSeed} />
        <div className="flex items-center gap-1">
          <p className="hidden md:block font-display text-paper text-lg tracking-tight mr-2">DR1FT</p>
          <a href="/messages" className="touch-target flex items-center gap-1.5 font-mono text-xs text-ash px-2 rounded-lg hover:bg-paper/10">
      ...
      <div className="md:max-w-4xl md:mx-auto md:grid md:grid-cols-[1fr_280px] md:gap-8 md:items-start md:px-6 md:py-6">
        <div className="max-w-md mx-auto md:max-w-none py-4 px-3 md:px-0 space-y-4">
          {initialItems.length === 0 && <p className="text-ash text-sm font-body text-center py-16">Für dich sind gerade keine Szenarien freigeschaltet. Frag deine Lehrkraft, ob eure Klasse schon ein Szenario zugewiesen bekommen hat.</p>}
          ...
          <div className="md:hidden bg-ink-light border border-ink-border rounded-card p-4 mt-2"><CompetencyPanel initial={competencyDisplay} /></div>
        </div>
        <aside className="hidden md:block sticky top-24 bg-ink-light border border-ink-border rounded-card p-5"><CompetencyPanel initial={competencyDisplay} /></aside>
      </div>
