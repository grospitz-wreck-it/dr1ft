...
export default async function FeedPage() {
  ...
  if (classInstanceError || !classInstanceId) {
    return <main className="min-h-screen flex items-center justify-center text-ash font-body px-6 text-center">Du bist aktuell keiner DR1FT-Klasse zugeordnet.</main>;
  }
  ...
  const { data: currentProfile } = await supabase
    .from("user_profiles")
    .select("display_name, username, avatar_seed, birth_year")
    .eq("id", user.id)
    .maybeSingle();
  ...
  for (const like of instanceLikes ?? []) {
    ...
    if (!list.some((p) => p.id === profile.id)) list.push({
      id: profile.id,
      displayName: profile.display_name ?? "DR1FT User",
      username: profile.username ?? "user",
      avatarSeed: profile.avatar_seed ?? profile.id,
    });
  ...
  const ctx: FeedContext = {
    ...
    recentTechniques: [],
    assignedScenarioIds,
    signalRatio: computeAdaptiveSignalRatio(progress),
  };
  ...
  return <FeedClient
    ...
    profile={{
      displayName: currentProfile?.display_name ?? "DR1FT User",
      username: currentProfile?.username ?? "user",
      avatarSeed: currentProfile?.avatar_seed ?? user.id,
    }}
  />;
}
