-- ============================================================
-- DR1FT — Actor Unification + NPC module availability + RLS
-- ============================================================
...
and not exists (
  select 1
  from public.actor_profiles a
  where a.legacy_creator_id = c.id
);
...
drop policy if exists "actor profiles readable"
...
-- ------------------------------------------------------------
-- NPC actor type + module assignments
-- ------------------------------------------------------------
...
drop policy if exists "npc module assignments readable"
  on public.npc_module_assignments;

create policy "npc module assignments readable"
  on public.npc_module_assignments
  for select
  using (true);

comment on table public.npc_module_assignments is
  'Global actor availability per DR1FT module/scenario. No class-instance ownership.';

comment on column public.npc_profiles.actor_type is
  'Role of the global digital-world actor.';

-- ------------------------------------------------------------
-- Editorial NPC write policies
-- ------------------------------------------------------------
...
drop policy if exists "staff manage npc profiles"
...
drop policy if exists "staff manage npc module assignments"
  on public.npc_module_assignments;
...
drop policy if exists "staff manage npc generation runs"
