...
where class_instance_id is null and payload ? 'classInstanceId';
...
begin
  ...
  loop
    ...
    from public.user_competency_progress
    ...
    v_new_evidence := v_current_evidence || jsonb_build_array(jsonb_build_object('missionId', v_mission.id, 'missionSlug', v_mission.slug, 'completedAt', now()));
    ...
    on conflict (user_id, competency_id, class_instance_id)
    do update set evidence = v_new_evidence, level = v_new_level, updated_at = now();
...
begin
  ...
  for v_step in
    ...
    from public.story_arc_steps sas
...
begin
  for v_arc in select id from public.story_arcs where scenario_id = new.scenario_id and status = 'live'
  loop
    for v_student in select user_id from public.class_instance_memberships where class_instance_id = new.class_instance_id and role = 'student' and left_at is null
    loop
...
begin
  ...
  for v_mission in select * from public.missions where scenario_id = v_scenario_id and status = 'live'
  loop
