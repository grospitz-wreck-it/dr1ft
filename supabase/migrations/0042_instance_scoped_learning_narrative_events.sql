...
where class_instance_id is null
  and payload ? 'classInstanceId';

create index if not exists idx_domain_events_user_instance
  on public.domain_events(user_id, class_instance_id, created_at desc);
...
begin
  ...
  for v_step in
    ...
    join public.user_story_arc_progress uap
      on uap.arc_id = sas.arc_id
     and uap.user_id = new.user_id
     and uap.class_instance_id = new.class_instance_id
